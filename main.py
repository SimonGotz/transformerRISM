...
def update_embeddings(transformer):
    ...
    for i in range(len(corpus.samefam)):
        for j in range(len(corpus.samefam)):
            with torch.no_grad():
...
def train_network_online(transformer, margin, lr, epoch, batch_size, hard_triplets=False):
    ...
    for i in range(iterations):
        ...
        elapsed = time.time() - start_time
        if i % log_interval == 0:
            print('| epoch {:3d} | batch {:3d} | {:5d}/{:5d} triplets | lr {:.2E} | ms/batch {:5.2f} | '
                        'loss {:5.4f}'.format(
...
def main(lr, d_model, nheads, n_layers, d_ff, name, features, mode="incipit", load=-1, hard_triplets=False):
    ...
    transformer = model.Transformer(src_vocab_size=10000, d_model=d_model, num_heads=nheads, num_layers=n_layers, d_ff=d_ff, max_seq_length=corpus.seqLen, dropout=dropout)
    lr_delta = lr / 10
    ...
    try: 
        ...
        with open("../Results/Tuning(jan2025)/{}Train.txt".format(name), "w") as f:
        ...
        if not best_val_loss or val_loss < best_val_loss:
            with open("../Results/Tuning(jan2025)/{}.pt".format(name), 'wb') as f:
    ...
    with open("../Results/Tuning(jan2025)/{}.pt".format(name), 'rb') as f:
    ...
    with open("../Results/Tuning(jan2025)/{}Test.txt".format(name), 'w') as f:
